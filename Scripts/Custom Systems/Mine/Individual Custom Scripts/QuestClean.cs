...
namespace Server.Custom
{
    ...
    public class QuestCleanGump : CG
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (button == 3)
            {
                sender.Mobile.SendGump(new QuestCleanGump(sender.Mobile, mPlayer, mPage + 1, 0, xOffset, yOffset));
            }
            else if (button == 4)
            {
                sender.Mobile.SendGump(new QuestCleanGump(sender.Mobile, mPlayer, mPage - 1, 0, xOffset, yOffset));
            ...
            {
                sender.Mobile.SendGump(new QuestCleanGump(sender.Mobile, mPlayer, mPage, mIndex + 1, xOffset, yOffset));
            }
            else if (button == 14)
            {
                sender.Mobile.SendGump(new QuestCleanGump(sender.Mobile, mPlayer, mPage, mIndex - 1, xOffset, yOffset));
            ...
            else if (button >= 2000)
            {
                ...
                sender.Mobile.SendGump(new QuestCleanGump(sender.Mobile, mPlayer, mPage, 0, xOffset, yOffset));
                sender.Mobile.SendGump(new DisplaySuccessGump(xOffset, yOffset, message));
            }
            else if (button >= 1000)
            {
    ...
    public class DisplaySuccessGump : CG
    {
            ...
            : base(x, y)
        {
            ...
            AddHtml(13, 63, 274, 74, message, 0x481, false, false);
